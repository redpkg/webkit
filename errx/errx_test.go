...
func TestSetInternal(t *testing.T) {
	...
	code := "59a7fc0a"
	message := "test set internal"
	err := errx.New(code, message).SetInternal(inner)

	assert.Equal(code, err.Code)
	assert.Equal(message, err.Message)
	assert.Equal(http.StatusInternalServerError, err.StatusCode)
	assert.Equal(err.Error(), fmt.Sprintf("(%s) %s | %s", err.Code, err.Message, inner.Error()))
}
...
func TestCodex(t *testing.T) {
	...
	inner := errors.New("foobar")
	code1 := "932c1223"
	err1 := errx.Codex(code1, inner)

	assert.Equal(code1, err1.Code)
	assert.Equal("Internal server error", err1.Message)
	assert.Equal(http.StatusInternalServerError, err1.StatusCode)
	assert.Equal(err1.Error(), fmt.Sprintf("(%s) %s | %s", err1.Code, err1.Message, inner.Error()))

	code2 := "2c321293"
	err2 := errx.Codex(code2, nil)

	assert.Equal(code2, err2.Code)
	assert.Equal("Internal server error", err2.Message)
	assert.Equal(http.StatusInternalServerError, err2.StatusCode)
	assert.Equal(err2.Error(), fmt.Sprintf("(%s) %s", err2.Code, err2.Message))
}
